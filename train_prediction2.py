...
# 与 ``--emg_channels`` 默认 ``0,6`` 配合：两路 EMG → ``in_channels=2`` 的 ResNet18
RESNET18_2CH_TYPES: frozenset[str] = frozenset({"resnet18_2ch", "res18_2ch", "resnet18_emg2"})
...
def collate_two_emg(emg_indices: tuple[int, ...]):
    """Stack batch then take only ``emg_indices`` → ``(B, len(indices), T)``."""

    def _collate(batch: list[dict[str, Any]]) -> tuple[torch.Tensor, torch.Tensor]:
        x9 = torch.stack([b["x"] for b in batch], dim=0)
        idx = list(emg_indices)
        x = x9[:, idx, :].contiguous()
...
def parse_emg_channels(spec: str | None) -> set[int] | None:
    """Parse e.g. ``'0,2'`` or ``'0, 2'`` → ``{0, 2}``. ``None``/empty/``all`` → use all 9 channels (no masking)."""
    if spec is None or not str(spec).strip():
        return None
    s0 = str(spec).strip().lower()
    if s0 in ("all", "full", "9", "none"):
        return None
    out: set[int] = set()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="0,6",
        help="For resnet18_2ch: exactly two indices (default 0,6). For 9-ch models: subset zeroed; use 'all' for full 9.",
    )
...
def main() -> None:
    ...
    use_two_ch = args.model_type in RESNET18_2CH_TYPES
    if use_two_ch:
        if emg_active is None or len(emg_active) != 2:
            raise ValueError(
                "model_type resnet18_2ch requires exactly two indices in --emg_channels "
                f"(e.g. 0,6); got {args.emg_channels!r}."
            )
        emg_slice: tuple[int, ...] = tuple(sorted(emg_active))
        collate_emg = collate_two_emg(emg_slice)
        emg_for_mask: set[int] | None = None
    else:
        emg_slice = None
        collate_emg = collate_batch
        emg_for_mask = emg_active
    ...
    if is_sklearn_model_type(args.model_type):
        ...
        if emg_for_mask is not None:
            X_tr, y_tr = dataloader_to_xy_arrays_masked(train_loader_fit, emg_for_mask)
        else:
            X_tr, y_tr = dataloader_to_xy_arrays(train_loader_fit)
        model.fit(X_tr, y_tr)
        tr_m = eval_one_epoch(model, train_loader_fit, cpu_dev, emg_active_channels=emg_for_mask)
        va_m = eval_one_epoch(model, val_loader, cpu_dev, emg_active_channels=emg_for_mask)
        te_m = eval_one_epoch(model, test_loader, cpu_dev, emg_active_channels=emg_for_mask)
        ...
        torch.save(
            {
                ...
                "emg_active_channels": sorted(emg_for_mask) if emg_for_mask is not None else None,
                "in_channels": 9,
                "emg_slice_indices": None,
            },
            ckpt_path,
        )
        ch_s = "all 9" if emg_for_mask is None else ",".join(str(c) for c in sorted(emg_for_mask))
        print(
        ...
        ch_s = (
            f"2-ch ResNet18 slice {list(emg_slice)}"
            if use_two_ch
            else ("all 9" if emg_for_mask is None else ",".join(str(c) for c in sorted(emg_for_mask)))
        )
        print(
        ...
        for ep in range(1, epochs + 1):
            tr_m, global_step = train_one_epoch(
                ...
                emg_active_channels=emg_for_mask,
            )
            va_m = eval_one_epoch(model, val_loader, device, emg_active_channels=emg_for_mask)
            te_m = eval_one_epoch(model, test_loader, device, emg_active_channels=emg_for_mask)
                ...
                torch.save(
                    {
                        ...
                        "in_channels": 2 if use_two_ch else 9,
                        "emg_slice_indices": list(emg_slice) if use_two_ch else None,
                    },
                    ckpt_path,
                )
            else:
    ...
    if not args.no_train_test_scatter and ckpt_path.is_file():
        ...
        scatter_loader = test_loader
        scatter_emg: set[int] | None = None
        if ck.get("backend") == "sklearn":
            ...
            scatter_emg = set(int(c) for c in _emg_ck) if _emg_ck is not None else None
        else:
            model = model.to(scatter_dev)
            model.load_state_dict(ck["model"])
            in_ch = int(ck.get("in_channels", 9))
            si = ck.get("emg_slice_indices")
            if in_ch == 2 and si is not None:
                scatter_loader = DataLoader(
                    ...
                    num_workers=num_workers,
                    pin_memory=torch.cuda.is_available(),
                    collate_fn=collate_two_emg(tuple(int(i) for i in si)),
                )
                scatter_emg = None
            else:
                _emg_ck = ck.get("emg_active_channels")
                scatter_emg = set(int(c) for c in _emg_ck) if _emg_ck is not None else None
        yt_te, yp_te = collect_predictions(model, scatter_loader, scatter_dev, emg_active_channels=scatter_emg)
